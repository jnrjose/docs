---
title: "Rate limit check (local only)"
api: "GET /v1/limit-check"
description: "Check current rate limiting status - only available in local environment"
---

## Overview

Check current rate limiting status - only available in local environment
...
<ResponseField name="message" type="string">
  Not available in non-local environment
</ResponseField>
...
const response = await fetch('https://api.railsfromthecrypt.com/v1/limit-check', {
  method: 'GET',
...
response = requests.get(
    'https://api.railsfromthecrypt.com/v1/limit-check'
)
