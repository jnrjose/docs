...
title: "Update user"
api: "PUT /v1/users/{id}"
description: "Update an existing user account (requires level 10 access)"
---

## Overview

Update an existing user account (requires level 10 access)
...
<ResponseField name="message" type="string">
  Forbidden - Insufficient user level
</ResponseField>
...
```bash cURL
curl -X PUT https://api.railsfromthecrypt.com/v1/users/{id} \
...
const response = await fetch('https://api.railsfromthecrypt.com/v1/users/{id}', {
  method: 'PUT',
  headers: {
...
response = requests.put(
    'https://api.railsfromthecrypt.com/v1/users/{id}',
