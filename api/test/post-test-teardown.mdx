---
title: "Clean up test environment"
api: "POST /v1/test/teardown"
description: "Remove all test data from the system. Requires teardown key. Only available in non-production environments."
---

## Overview

Remove all test data from the system. Requires teardown key. Only available in non-production environments.

## Request

<ParamField header="x-teardown-key" type="string" required>
  Teardown key for test environment cleanup
</ParamField>
...
<ResponseField name="message" type="string">
  Invalid credentials or production environment
</ResponseField>
...
```bash cURL
curl -X POST https://api.railsfromthecrypt.com/v1/test/teardown \
  -H "Content-Type: application/json"
```

```javascript JavaScript
const response = await fetch('https://api.railsfromthecrypt.com/v1/test/teardown', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://api.railsfromthecrypt.com/v1/test/teardown',
